...
public abstract class AbstractServiceController {
     ...
     *
     */
    Logger logger = Logger.getLogger(AbstractServiceController.class.getName());
    protected static final String SCHEME_REQUEST_PARAM_KEY = "scheme";
    protected static final String FRAGMENT_REQUEST_PARAM_KEY = "fragment";
    ...
    protected String extractValueFromRequestParam (HttpServletRequest request, String param ) throws InvalidRequest {
        debugRequest(request);
       // get params from request
        String value = request.getParameter(param);
        if (value != null) {
            try {
                value = EncodingUtilities.decodeString(request.getParameter(param));
            } catch (UnsupportedEncodingException ex) {
               throw new InvalidRequest("4200", "Incorrectly encoded " + param + " parameter: " + ex.getMessage());
            }
        } else {
            throw new NullPointerException( "Request missing " + param + " parameter" );
        }
        return value;
    }

    protected Identifier extractPidFromRequestParam (HttpServletRequest request ) throws InvalidRequest {
                // get params from request
        Identifier pid = null;
        try {
    ...
    protected Subject extractSubjectFromRequestParam(HttpServletRequest request) throws InvalidRequest {
    ...
    /* resource path should include the ending slash */
    protected Identifier extractPidFromRequestURI(HttpServletRequest request, String resourcePath) throws InvalidRequest {
        Identifier pid = null;
        try {
        	String requestUri = request.getRequestURI();

                
        	String pidString = requestUri.substring(requestUri.lastIndexOf(resourcePath) + resourcePath.length());
